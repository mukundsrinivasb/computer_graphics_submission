...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    Eigen::Matrix3f A;
    A <<  v0.x - v1.x, v0.x - v2.x, ray.direction.x,
            v0.y - v1.y, v0.y - v2.y, ray.direction.y,
            v0.z - v1.z, v0.z - v2.z, ray.direction.z;
    
    Eigen::Matrix3f betaMatrix;
    betaMatrix <<  v0.x - ray.origin.x, v0.x - v2.x, ray.direction.x,
            v0.y - ray.origin.y, v0.y - v2.y, ray.direction.y,
            v0.z - ray.origin.z, v0.z - v2.z, ray.direction.z;
    
    Eigen::Matrix3f gammaMatrix;
    gammaMatrix <<  v0.x - v1.x, v0.x - ray.origin.x, ray.direction.x,
            v0.y - v1.y, v0.y - ray.origin.y, ray.direction.y,
            v0.z - v1.z, v0.z - ray.origin.z, ray.direction.z;

    Eigen::Matrix3f tMatrix;
    tMatrix << v0.x - v1.x, v0.x - v2.x, v0.x - ray.origin.x,
                v0.y - v1.y, v0.y - v2.y, v0.y - ray.origin.y,
                v0.z - v1.z, v0.z - v2.z, v0.z - ray.origin.z;
    
    auto beta = betaMatrix.determinant() / A.determinant();
    auto gamma = gammaMatrix.determinant() / A.determinant();
    auto t = tMatrix.determinant() / A.determinant();

    Intersection i = Intersection();

    if (!(t >= 0 && (beta >= 0 && beta <= 1) && (gamma >= 0 && gamma <= 1) && (1 - beta - gamma >= 0 && 1 - beta - gamma <= 1))) {
        return i;
    }

    i.happened = true;
    i.coords = Vector3f(ray.origin.x + ray.direction.x * t, ray.origin.y + ray.direction.y * t, ray.origin.z + ray.direction.z * t);
    i.tcoords = Vector2f(beta, gamma);
    i.normal = normal;
    i.tnear = sqrt(pow(i.coords.x - ray.origin.x, 2) + pow(i.coords.y - ray.origin.y, 2) + pow(i.coords.z - ray.origin.z, 2));
    i.obj = this;
    i.material = m;
